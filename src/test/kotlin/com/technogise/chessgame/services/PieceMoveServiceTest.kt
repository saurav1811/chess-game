...
class PieceMoveServiceTest {
 ...
 @Nested
 inner class WithoutOpponentStartPositionLabel {
  @Test
 ...
 @Nested
 inner class WithOpponentStartPositionLabel {
  @Test
  fun `SHOULD return all next positions for ROOK from given start position with opponentPositionLabel WHEN all these positions are valid`() {
   val pieceMoveService = PieceMoveService()

   val validNextPositions = pieceMoveService.nextValidPositionsFor(ROOK, "D5", "D7")

   validNextPositions shouldContainExactlyInAnyOrder listOf("D1", "D2", "D3", "D4", "D6", "D7", "A5", "B5", "C5", "E5", "F5", "G5", "H5")
  }

  @Test
  fun `SHOULD return all next positions for QUEEN from given start position with opponentPositionLabel WHEN all those positions are valid`() {
   val pieceMoveService = PieceMoveService()

   val validNextPositions = pieceMoveService.nextValidPositionsFor(QUEEN, "A8", "C6")

   validNextPositions shouldContainExactlyInAnyOrder listOf("A1", "A2", "A3", "A4", "A5", "A6", "A7", "B8", "C8", "D8", "E8", "F8", "G8", "H8", "B7", "C6")
  }
 }
}